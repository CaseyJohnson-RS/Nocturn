...
Модули без эндпоинтов (rag) содержат только service и repository.
...
|notes|CRUD заметок, soft delete, корзина, версионирование. При сохранении пишет задачу в embedding_queue|Да|
...
|rag|Семантический поиск, полнотекстовый поиск, комбинирование результатов|Нет|
<div class="page-break" style="page-break-before: always;"></div>
...
|nickname|VARCHAR(32)|UNIQUE, NOT NULL|Уникальность регистронезависимая|
...
|embedding|VECTOR|NOT NULL|Размерность определяется моделью|
|is_deleted|BOOLEAN|NOT NULL, DEFAULT false|Синхронизируется с soft delete заметки|
...
|status|VARCHAR(15)|NOT NULL, DEFAULT 'pending'|pending / processing / failed|
...
|note_embeddings|embedding WHERE is_deleted = false|HNSW|Семантический поиск по активным заметкам|
|note_embeddings|user_id, is_deleted|B-tree|Фильтрация при поиске|
...
Таблица `note_embeddings` содержит `user_id` и `is_deleted`, дублирующие данные из `notes`. Обоснование: все поисковые запросы фильтруют по user_id и is_deleted = false. Прямые поля позволяют использовать составной индекс без JOIN с таблицей notes. При soft delete заметки `is_deleted` в note_embeddings устанавливается в true. При восстановлении - обратно в false.

### 5.6 Каскадное удаление

|Событие|Поведение|
|---|---|
|Soft delete заметки|is_deleted устанавливается в true в note_embeddings. Теги (note_tags) сохраняются для возможности восстановления|
|Восстановление заметки|is_deleted устанавливается в false в note_embeddings|
|Hard delete заметки|Каскадно удаляются note_tags, note_embeddings, embedding_queue|
...
|Размерность|≤ 1024| Баланс между качеством и объёмом хранения ([[CPS#5.4 Очередь embedding]])     |
...
|Сценарий|Чанков/заметка|Всего чанков|Объём векторов|
|---|---|---|---|
|Реалистичный|~2|~150 000|~375 МБ|
|Worst case|~15|~1 125 000|~2.6–3.3 ГБ|
...
|Partial index|WHERE is_deleted = false|Исключает удалённые заметки из поиска без runtime-фильтрации|
...
- При успешной генерации: старые чанки удаляются, новые записываются в note_embeddings, задача удаляется из очереди
...
При soft delete заметки вектора не удаляются из note_embeddings - поле is_deleted устанавливается в true. Partial index (WHERE is_deleted = false) исключает их из поиска. При восстановлении заметки - is_deleted возвращается в false, вектора снова доступны для поиска без пересчёта.
...
Ответы ассистента поступают через общее SSE-соединение (см. 8.7). Типы событий и формат payload определяются в [[AIS#11. SSE-события]]. Proposals отображаются как интерактивные элементы после завершения стриминга.

<div class="page-break" style="page-break-before: always;"></div>

### 8.7 SSE-соединение и presence

Клиент при загрузке приложения открывает persistent SSE-соединение к backend. Соединение выполняет три функции: доставка AI-токенов (см. 8.6), поддержание presence активного клиента и получение управляющих событий.

**Типы событий**

| Тип                   | Назначение                                                                                     |
| --------------------- | ---------------------------------------------------------------------------------------------- |
| `ping`                | Поддержание соединения, обнаружение отключения клиента. Интервал — [[CPS#5.1 Сессии и токены]] |
| `session_taken`       | Активность перехвачена другим клиентом, переход в read-only                                    |
| События AI-ассистента | Стриминг ответов, proposals. Типы определяются в [[AIS#11. SSE-события]]                       |

**Presence**

При установке SSE-соединения backend регистрирует presence-ключ в Redis с TTL ([[CPS#5.1 Сессии и токены]]). Сервер периодически пишет `ping` в SSE-поток. При закрытии вкладки или потере сети запись `ping` вызывает broken pipe — сервер удаляет presence-ключ. TTL служит страховкой на случай краша backend.

**Подключение второго клиента**

При открытии SSE-соединения backend проверяет наличие presence-ключа. Если активный клиент уже существует, новый клиент переходит в read-only режим: доступен просмотр заметок и истории AI-чата, редактирование, создание, удаление заметок и отправка сообщений в AI-чат заблокированы. Клиент отображает баннер с кнопкой «Продолжить здесь».
...
При потере SSE-соединения клиент автоматически переподключается. При переподключении presence проверяется заново — клиент может оказаться как активным, так и в read-only.
...
|Задача|Описание|
|---|---|
|Обработка embedding_queue|Берёт задачи со статусом `pending`, генерирует вектора через RouterAI, записывает в note_embeddings|
...
|Hard delete заметок|Удаляет notes с deleted_at старше срока ([[CPS#5.2 Заметки]]) с каскадным удалением note_tags, note_embeddings, embedding_queue|
...
6. Удалить старые записи из note_embeddings для этой заметки
7. Записать новые чанки с векторами в note_embeddings
...
| 3   | Денормализация user_id и is_deleted в note_embeddings                                   | JOIN с notes при каждом поиске                  | Все поисковые запросы фильтруют по user_id и is_deleted. Прямые поля позволяют использовать составной индекс без JOIN. Стоимость - синхронизация is_deleted при soft delete/восстановлении                                                                |
